...
expName = 'Facial expression recognition task '  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/beth/Desktop/Facial expression recognition task/Facial expression recognition task _lastrun.py',
...
movie = visual.MovieStim(
    ...
    pos=(0, 0.15), size=(0.5,0.5), units=None,
    ori=0.0, anchor='center',opacity=None, contrast=1.0,
)
text = visual.TextStim(win=win, name='text',
    text='1: Anger       2: Disgust       3: Fear\n4: Happiness   5: Sadness       6: Surprise',
    font='Open Sans',
...
introComponents = [text_instr, key_resp]
...
trials_2 = data.TrialHandler(nReps=1.0, method='fullRandom', 
    ...
    trialList=data.importConditions('facial stimuli.csv'),
...
for thisTrial_2 in trials_2:
    ...
    routineForceEnded = False
    # update component parameters for each repeat
    movie.setMovie(face_images)
    key_resp_2.keys = []
    key_resp_2.rt = []
    _key_resp_2_allKeys = []
    # keep track of which components have finished
    Trial1Components = [movie, text, key_resp_2, ISI]
    ...
    while continueRoutine:
        ...
        # *movie* updates
        if movie.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
            # keep track of start time/frame for later
